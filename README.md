...
- 드라마에 **하루 편수**(`episodes_per_day`)를 2 이상으로 지정하면, 한 방영일에 그 편수만큼 실제로 받아야 그 요일이 `Y`로 체크됩니다(요일별 카운터 `day_progress`). 신병처럼 수/목 각 2편씩 올라오는 드라마가 수요일 2편만 받았을 때 목요일까지 체크로 넘어가던 문제를 막습니다. 카운터는 주간 리셋 때 해당 요일만 0으로 돌아갑니다.
